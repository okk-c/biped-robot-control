...
class LeggedRobot(BaseTask):  # 定义四足机器人任务类，继承自BaseTask
    ...
    def create_sim(self):
        """Overridden create_sim to load procedural terrain instead of flat ground."""
        self.up_axis_idx = 2
    ...
    def _create_terrain(self):
        """
        Final version: correct fields for your custom gym_38 HeightFieldParams.
        """

        print("[Terrain] Using correct HeightFieldParams layout for gym_38")

        # Import your terrain generator
        from legged_gym.utils.terrain import Terrain as TerrainFactory

        # Build terrain
        terrain_obj = TerrainFactory(self.cfg.terrain, self.num_envs)
        hs = np.asarray(terrain_obj.height_field_raw, dtype=np.int16)  # (H, W)

        H, W = hs.shape
        print(f"[Terrain] Heightmap shape = {hs.shape}, min={hs.min()}, max={hs.max()}")

        # Flatten to 1D for IsaacGym
        hs_flat = hs.reshape(-1).astype(np.int16)

        # Get horizontal & vertical scales from cfg
        # row_scale corresponds to Y direction, column_scale for X direction
        row_scale = float(self.cfg.terrain.horizontal_scale)
        column_scale = float(self.cfg.terrain.horizontal_scale)
        vertical_scale = float(self.cfg.terrain.vertical_scale)

        print(f"[Terrain] row_scale={row_scale}, column_scale={column_scale}, vertical_scale={vertical_scale}")

        # Safety checks
        row_scale = max(row_scale, 0.0001)
        column_scale = max(column_scale, 0.0001)
        vertical_scale = max(vertical_scale, 0.0001)

        # Create HeightFieldParams with correct fields
        params = gymapi.HeightFieldParams()
        params.nbRows = H
        params.nbColumns = W
        params.row_scale = row_scale
        params.column_scale = column_scale
        params.vertical_scale = vertical_scale
        params.static_friction = 1.0
        params.dynamic_friction = 1.0
        params.restitution = 0.0

        print("[Terrain] HeightFieldParams:")
        print(f"  nbRows={params.nbRows}, nbColumns={params.nbColumns}")
        print(f"  row_scale={params.row_scale}, column_scale={params.column_scale}")
        print(f"  vertical_scale={params.vertical_scale}")

        # Try adding heightfield
        try:
            self.gym.add_heightfield(
                self.sim,
                np.ascontiguousarray(hs_flat, dtype=np.int16),
                params
            )

            print("[Terrain] add_heightfield SUCCESS ✔")
        except Exception as e:
            print("[Terrain] add_heightfield FAILED:", e)
            print("[Terrain] Falling back to plane")
    ...
    def _get_env_origins(self):
        """
        Overridden version:
        If using procedural heightfield terrain, place robots at the terrain center.
        Otherwise fallback to default grid layout.
        """

        # ----------------------------------------
        # Case 1: 如果 terrain.py 提供了 height_field_raw → 使用 heightfield 中心
        # ----------------------------------------
        try:
            from legged_gym.utils.terrain import Terrain
            # terrain object is built during _create_terrain()
            # we store it as self.terrain_obj from there
            if hasattr(self, "terrain_obj") and self.terrain_obj.height_field_raw is not None:
                H, W = self.terrain_obj.height_field_raw.shape
                hs = self.terrain_obj.height_field_raw.astype(float)

                # 计算地形中心
                center_x = (W * self.cfg.terrain.horizontal_scale) * 0.5
                center_y = (H * self.cfg.terrain.horizontal_scale) * 0.5

                # 取中心附近平均高度
                cx, cy = int(W/2), int(H/2)
                base_h = hs[max(0,cx-1):cx+2, max(0,cy-1):cy+2].mean() * self.cfg.terrain.vertical_scale

                # ----------------------------------------
                # 设置 env 原点（全部放在 heightfield 正中央）
                # ----------------------------------------
                self.env_origins = torch.zeros(self.num_envs, 3, device=self.device)
                self.env_origins[:, 0] = center_x
                self.env_origins[:, 1] = center_y
                self.env_origins[:, 2] = base_h + 0.5  # 抬高 0.5m 防止穿地

                print("⭐[Terrain] Using heightfield center spawn")
                print("center =", center_x, center_y, " height =", base_h)
                return
        except:
            pass

        # ----------------------------------------
        # Case 2: 默认 flat grid (原来的逻辑)
        # ----------------------------------------
        ...
        print("⚠ Using DEFAULT grid origins (no terrain)")
